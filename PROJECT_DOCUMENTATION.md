...
> Status: **all four build phases complete and verified live** (see §9). This document records *why* every decision was made; see [PLANNING.md](./PLANNING.md) for the concrete, step-by-step build plan (*what to do, in what order*).
> **Audited for doc/code drift on 2026-09-05** — see §5.16 for what was found and fixed.
...
- **Confidence Scoring** — each detection carries a `confidence` value (used internally by severity logic — §5.2), but it is *not* currently surfaced to the end user as a distinct displayed metric anywhere in the report or UI. Corrected 2026-09-05 — this was previously overstated (see §5.16).
...
**Correction (2026-09-05):** this section previously claimed each node "tries `langgraph.prebuilt.create_react_agent(model, tools, prompt)` first." That was never true of Aegis's own code — `create_react_agent` is the *reference tutorial's* (OpenAI-based) approach, not something Aegis ever calls. Aegis's actual pattern: deterministic Python tools do the real detection/classification/auto-block work, and Gemini is called directly via `agent_loop.py`'s `simple_generate()` for narrative/critique/report text on most nodes. The one exception is Classify's GraphRAG lookup, which uses the full manual function-calling loop (`run_tool_agent()`) to let Gemini itself decide which findings warrant a CVE lookup — see §5.16 for how that got wired in.
...
- **The only autonomous action:** for any finding that is (a) IP-based, (b) `High` severity, and (c) confirmed malicious by threat intel (`reputation == "malicious"` AND `risk_score` above a configurable threshold, default **80.0** on AbuseIPDB's native 0-100 scale — corrected here 2026-09-05 from a stale "8.0" left over from before the Phase 2 scale change, §5.13), automatically add that IP to a **local blocklist file** (`blocklist.json`) with a timestamp and the triggering finding as the reason. *(Refined during Phase 1 build/testing — the gate originally compared pattern-detection confidence to a threshold, which would almost never fire since no detector emits confidence that high; risk_score is the correct "confirmed malicious" signal, see §5.12 Build Notes below.)*
...
- **Not a ReAct agent** — calls Gemini directly for report generation via `agent_loop.simple_generate(...)` (no separate `_llm_report` wrapper function exists — corrected here 2026-09-05, an earlier draft of this doc named one that was never actually written), with a deterministic fallback `_fallback_report(findings, autonomous_actions)` when no API key is set, an LLM error occurs, or there are simply no findings (returns `"No suspicious activity detected."`).
...
### 5.10 GraphRAG Layer — CVE/CWE/CAPEC/ATT&CK Knowledge Retrieval ✅ BUILT & VERIFIED LIVE (2026-09-05)
...
- **What it retrieves:** relevant CVE entries, their associated CWE (Common Weakness Enumeration) categories, and mapped MITRE ATT&CK techniques — matched against the patterns/anomalies the Detect agent finds. E.g. a detected `privilege_escalation` pattern doesn't just get "similar-sounding" CVEs back — it gets the actual graph neighborhood.
- **Knowledge graph structure (as actually built — one node type added versus the original plan):** `CVE -[HAS_WEAKNESS]-> CWE -[RELATED_TO]-> CAPEC -[MAPS_TO]-> AttackTechnique`. Research during implementation (2026-09-05) found there is **no direct, official CWE-to-ATT&CK mapping published anywhere** — CAPEC (Common Attack Pattern Enumeration and Classification) is the real, documented bridge MITRE itself uses. The original design (§5.10 as first written) assumed a direct `CWE -[MAPS_TO]-> ATT&CK` edge; this was corrected before writing any loading code, not discovered as a bug afterward.
- **Graph store:** **Neo4j Aura** (free tier, cloud-hosted) — chosen over local Docker since Docker Desktop wasn't running and Aura needed no local setup. One real gotcha: Aura's actual database name is the instance ID (e.g. `e46137b0`), **not** literally `"neo4j"` — that's a local-Docker-only convention. Code doesn't hardcode a database name; it lets the driver use its default ("home") database.
- **Embedding provider:** **Voyage AI** (`voyage-4-lite`) — kept as a dedicated embeddings provider even though Gemini has its own native embeddings API, deliberately decoupling embeddings from the LLM provider (§6). Real gotcha found live: Voyage AI's free tier without a payment method on file is capped at **3 requests/minute** — `tools/graphrag.py` retries with a 20s backoff and an in-run cache (`lru_cache`) to avoid re-embedding identical finding descriptions (common in practice — e.g. two `privilege_escalation` findings often share the same description text).
- **Retrieval strategy — hybrid, verified working:** (1) embed the finding's description via Voyage AI, vector-search the nearest CVE node(s) in Neo4j (`db.index.vector.queryNodes` — deprecated in favor of Cypher 25's `SEARCH` clause as of Neo4j 2026.04, but confirmed still functional; kept as-is since the new syntax's documented form is built for node-to-node similarity, not an ad-hoc query vector like ours), then (2) traverse `CVE->CWE->CAPEC->AttackTechnique` via Cypher for the surrounding context.
- **CVE/CWE/CAPEC/ATT&CK data source — real, not fabricated:** `scripts/build_cve_graph_data.py` pulls real CVEs from the **NVD CVE API v2.0** (`cweId` filter, no API key needed for this modest pull — public rate limit 5 req/30s) for four CWEs matched to Aegis's own detection patterns (CWE-307 brute force, CWE-269 privilege escalation, CWE-287 improper authentication, CWE-200 info exposure), plus MITRE's official **CAPEC "ATT&CK Related Patterns" CSV export** (`capec.mitre.org/data/csv/658.csv.zip`) for the CWE↔CAPEC↔ATT&CK edges — verified live to contain real ATT&CK technique IDs (e.g. `T1548`, `T1110.001`) per attack pattern. `scripts/load_cve_graph.py` embeds and loads this into Neo4j. Result: 60 real CVEs, 4 CWEs, 33 CAPEC patterns, 39 ATT&CK techniques.
- **Where it plugs in:** `tools/graphrag.py`'s `cve_graph_retrieval_tool`, called from `nodes/classify.py` for every finding. Its result can **escalate severity by one level** (Low→Medium, Medium→High, never downgrades) when a similar (`similarity >= 0.5`), high-CVSS (`>= 7.0`) real CVE is retrieved — verified live: a `privilege_escalation` finding that was `Medium` under rule-based logic alone became `High` once GraphRAG retrieved a real CVSS-7.8 CVE for it. This is the proof GraphRAG actually changes decisions, not just decorates the report (both escalation thresholds are placeholders pending further live tuning).
- **Surfaced in the report:** each finding's top CVE match (ID, CVSS score) and up to 2 distinct ATT&CK technique IDs/names are cited directly in the Markdown report's Findings section.
...
**Operational discovery — Gemini model quota:** `gemini-3.8-flash`'s free tier caps at 20 requests/day (hit mid-testing). `gemini-2.5-flash` returned "no longer available to new users" for this API key/project. Settled on `gemini-3.6-flash` since it was genuinely *accessible* to this key (unlike the other two) — **but correction, found later (2026-09-05, during a later session):** `gemini-3.6-flash`'s free tier is capped at 20 requests/day too, same as `gemini-3.8-flash`. The earlier claim that it had "a workable free-tier quota" was wrong — it just hadn't been hit yet by the point that was written. A single Aegis run makes up to 4 Gemini calls (Detect narrative, Classify rationale, Reflect critique, Report generation), so heavy same-day testing exhausts the quota quickly regardless of which current free-tier Gemini model is selected. `AEGIS_MODEL` remains a one-line `.env` change; a paid tier or a different model removes this ceiling.
...
### 5.14 Phase 3 Build Notes (2026-09-05) — GraphRAG, fully verified live

**Research corrected the design before any loading code was written:** the original plan (§5.10 as first drafted) assumed a direct `CWE -[MAPS_TO]-> ATT&CK` edge. Live research found no such official mapping exists anywhere — MITRE's real bridge is `CWE -> CAPEC -> ATT&CK`. The schema was corrected to include a `CAPEC` node type *before* writing `scripts/build_cve_graph_data.py`, not discovered as a bug afterward. CAPEC's own "ATT&CK Related Patterns" CSV export was verified (by actually downloading and inspecting it) to contain both CWE references and real ATT&CK technique IDs per attack pattern — confirming it was a viable, real bridge rather than assuming so.

**Real, non-fabricated dataset:** `scripts/build_cve_graph_data.py` pulled 60 real CVEs from the NVD API (no key required) and 43 real CWE↔CAPEC↔ATT&CK mapping rows from CAPEC's official data — 33 unique CAPEC patterns, 39 unique ATT&CK techniques. Every ID in Aegis's knowledge graph traces to a real MITRE/NIST source.

**Two real infrastructure gotchas found only by actually connecting:**
1. **Neo4j Aura's database is not named `"neo4j"`.** That's a local-Docker-only default. Aura's actual database name is the instance ID (e.g. `e46137b0`) — the loading script initially failed with `DatabaseNotFound` until this was discovered via `SHOW DATABASES` and fixed by not hardcoding a database name at all (letting the driver use its default "home" database).
2. **Voyage AI's free tier without a payment method on file is capped at 3 requests/minute** (not obvious from documentation alone — surfaced as a live `RateLimitError` mid-run). Fixed with retry-with-backoff and an in-run embedding cache, since Classify calls the retrieval tool once per finding and several findings often share identical description text.

**A robustness bug caught by testing, not by inspection:** the first version of `_embed_query` only checked "is a key configured," and crashed the whole pipeline on the rate-limit error above instead of degrading gracefully like every other integration in this project. Fixed to catch any exception and return `None`, matching the established pattern (Gemini, AbuseIPDB, Slack all already did this correctly — GraphRAG initially didn't).

**Severity escalation verified working, not just present in the code:** a `privilege_escalation` finding that rule-based logic alone classified as `Medium` became `High` once GraphRAG retrieved a real CVSS-7.8 CVE (`CVE-2008-2931`) for it — confirmed in an actual pipeline run, with the CVE ID and mapped ATT&CK technique (`T1548`, Abuse Elevation Control Mechanism) cited directly in the generated report. This is the proof required by the original Phase 3 Definition of Done: GraphRAG context measurably changes a decision, not just decorates the output.
...
### 5.16 `agent_loop.py`'s `run_tool_agent`/`results_for` — from built-but-unused to a real decision path (2026-09-06)

`run_tool_agent()` (a manual Gemini function-calling loop) and its helper `results_for()` were built and verified standalone early in Phase 1, but through Phase 3 every node called `simple_generate()` (no tools) instead — Classify's GraphRAG lookup ran a fixed, deterministic Cypher query for *every* finding rather than letting Gemini decide. That earlier phrasing in §4's correction note ("ended up unused") is now out of date.

**What changed:** `classify.py`'s `_llm_driven_graph_lookup()` now gives Gemini a single `cve_lookup` tool via `run_tool_agent` and a numbered list of findings, and lets it decide *per finding* whether a CVE/ATT&CK lookup is worth doing and what to search for — instead of always querying the graph with a fixed template string. `results_for` pulls the tool's actual return values back out of `calls_made` for use in severity assessment. Fast Mode (no Gemini client) keeps the old deterministic full-coverage lookup as a fallback, so nothing loses coverage when no LLM is configured.

**Verified live:** an isolated test against `gemini-3.7-flash` (a separate quota pool, since `gemini-3.6-flash` was quota-exhausted at the time) gave Gemini two findings — a `brute_force` pattern and a `foreign_login` anomaly — matching the prompt's guidance to skip generic findings with no specific CVE analog. Gemini correctly called `cve_lookup` only for index 0 (`brute_force`) and skipped index 1 (`foreign_login`), retrieving 3 real graph matches for the one it queried. A full pipeline run against the live API separately confirmed graceful degradation to the Fast Mode fallback when quota was exhausted.
...
| Graph store | **Neo4j Aura** (free tier, cloud) — chosen over local Docker | Docker Desktop wasn't running when Phase 3 started; Aura needed no local setup. Real gotcha: Aura's database name is the instance ID, not literally `"neo4j"` (§5.10). |
...
| CVE/CWE/CAPEC/ATT&CK data source | **Real data: NVD CVE API v2.0 (no key needed) + MITRE's official CAPEC "ATT&CK Related Patterns" CSV** | Fetched once via `scripts/build_cve_graph_data.py`, cached to `data/cve_graph_data.json`, loaded into Neo4j via `scripts/load_cve_graph.py`; no live dependency during pipeline runs. CAPEC turned out to be a required bridge, not an optional extra — see §5.10. |
...
| **Autonomous response** | **Yes — but narrowly bounded**, not the original concept's fully autonomous orchestration | Closes gap #2 against the original concept (§7a): the *only* autonomous action is auto-blocking a confirmed-malicious IP (High severity + `reputation == "malicious"` + `risk_score >= 80`, not "confidence" — corrected 2026-09-05, see §5.12). Every other action (password resets, system isolation, access reviews) stays a human-executed recommendation. |
...
| **Testing** | 55-test pytest suite covering the deterministic core (detection thresholds, the full severity-escalation ladder including graph/history escalation and the escalate-only guarantee, anonymization roundtrip, cross-run history persistence, the auto-block gate) — the safety-critical logic that has to be right regardless of which LLM is behind it. |
